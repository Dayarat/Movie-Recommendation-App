...
class Recommender:
    ...
    def recommend(self, movie_name: str) -> List[Movie]:
        """
        Find similar Movie to the given Movie name.
        Parameters:
        ...
        for i in similar_items[1:6]:
            ...
            book = Movie(
                ...
                movie_match_df['IMDB ID'].values[0],
                movie_match_df["Director"].values[0],
                movie_match_df["Year"].values[0],
                movie_match_df["Rating"].values[0],
                movie_match_df["Movie Poster"].values[0],
            )
            recommended_movies.append(book)
